...
const Dashboard = ({ user: admin }) => {
  ...
  const Appointments = async () => {
    try {
      const {data} = await axios.get("https://care-connect-p81e.onrender.com/api/v1/user/allpatient");
  ...
  const fetchAppointments = async () => {
    try {
      const { data } = await axios.get(
        "https://care-connect-p81e.onrender.com/api/v1/appointment/getall",
  ...
  const handleUpdateStatus = async (appointmentId, status) => {
    try {
      const { data } = await axios.put(
        `https://care-connect-p81e.onrender.com/api/v1/appointment/update/${appointmentId}`,
        { status },
        { withCredentials: true }
      );
